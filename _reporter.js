...
const [alignedOverlayedVisualized, alignedOldVisualized] = alignLayouts(
  overlayedVisualized,
  oldVisualized
);
...
if (
  !["🏴", "🏳️", "🏁", "🪦"].some((emoji) => reportNew.includes(emoji)) &&
  omitNew
)
  process.exit(1);
...
const directoryPath = path.join("./rustic_storage_delta", contractPath.dir);
...
function isSame(item1, item2) {
  ...
  if (
    ...
    JSON.stringify(item1.type) === JSON.stringify(item2.type)
  ) {
